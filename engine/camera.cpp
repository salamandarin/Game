...
void Camera::render(const Dungeon& dungeon) const {
    ...
    // draw doors
    for (auto& [position, door] : dungeon.doors) {
        if (within_view(position)) {
            render(position, door.get_sprite());
        }
    }
}

void Camera::render(const Actors& actors) const {
    for (auto actor : actors) {
        if (within_view(actor->get_position()) && actor->alive && actor->is_visible()) {
            for (auto sprite : actor->get_sprites()) {
                render(actor->get_position(), sprite);
            }
        }
    }
}
